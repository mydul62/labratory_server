  ...
  try {
    ...
    app.get('/alltests', async (req, res) => {
      try {
        ...
        const result = await AllTestCollection.find({ date: { $gte: formattedToday } }).sort({ date: 1 }).toArray();
        res.send(result);
      } catch (error) {
        console.error(error);
    ...
    app.patch('/allusers/Updates/update/:email', async (req, res) => {
      ...
      const updateInformations = {
        ...
        const result = await AlluserCollection.updateOne(query, updateInformations);
